...
GET-запрос к коллекции всегда возвращает объект с массивом текущей страницы и общим количеством записей после фильтрации. По умолчанию `_page` равен `1`, а `_perPage` — `Math.min(10, server.maxPageSize)` (`10` при стандартных настройках):
...
`null` и отсутствующие значения считаются равными пустыми значениями: по возрастанию они идут после остальных, по убыванию — перед ними. При равенстве применяется следующее правило сортировки; записи, равные по всем правилам, сохраняют исходный порядок.
...
При обычном запуске тела запросов проверяются по тем же автоматически выведенным и настроенным схемам. Для `POST` и `PUT` проверяются обязательные поля из `required`; `PATCH` проверяет только фактически переданные поля. Настройки `formats` и `properties` применяются ко всем трём методам. Не перечисленные дополнительные поля объекта остаются разрешёнными, если его схема не задаёт `additionalProperties: false`; при таком запрете лишние поля приводят к `400`, а не удаляются молча. Валидация сохраняет допустимые поля веток `anyOf` и `oneOf` независимо от их порядка. Некорректные тела возвращают `400` без изменения сохранённых данных.

Типы JSON проверяются без автоматического преобразования: строка `"1"` не соответствует схеме `integer`. Поле, содержащее только `null`, описывается как `{ "type": "string", "nullable": true, "enum": [null] }`: эта схема OpenAPI 3.0.3 принимает только `null`. При смешанных типах отдельная ветка для `null` добавляется в `oneOf`, если это необходимо.

Явный `type` в `properties` заменяет автоматически выведенное ограничение «только `null`», если переопределение не задаёт собственный `enum`. Например, `{ "type": "string", "nullable": true }` разрешает строки и `null`, даже если все исходные значения равны `null`. Выведенная допустимость `null` сохраняется, пока не задано `nullable: false`; явные перечисления `enum` всегда сохраняются. Это также действует для вложенных свойств и элементов массивов.

Явные вложенные ограничения `properties.required` сохраняются и объединяются с сокращёнными путями обязательных полей. Обработка обязательных путей, форматов и связей проходит через `oneOf`, `anyOf` и `allOf`, включая свойства рядом с ними. `PATCH` выполняет поверхностное слияние: отсутствующее поле верхнего уровня остаётся нетронутым, а переданный объект заменяет это поле и должен удовлетворять вложенным ограничениям. При валидации `POST` и `PUT` могут подставляться значения по умолчанию; `PATCH` их не подставляет, в том числе внутри переданных объектов и массивов.

Каждый экземпляр `createServer()` использует единый фиксированный контракт для валидации запросов и `server.openapi()`, выведенный из данных, прочитанных при создании, и настроенной схемы. CRUD и внешние правки файла по-прежнему меняют данные, читаемые API, но не этот контракт. Для определения новых типов полей или ресурсов создайте новый экземпляр сервера либо перезапустите CLI. Поля из изначально пустой коллекции остаются без ограничений, если они не описаны в `properties`. Каждый вызов `server.openapi()` возвращает независимую копию контракта.

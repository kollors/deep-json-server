...
Модели задаются в `models`. Корень схемы принимает `models`, `timestamps` и `softDelete`, но не `api`. У модели настройки дат и мягкого удаления могут переопределять общие значения. Секции конфигурации `openapi` и `graphql` включают соответствующий API. По умолчанию каждая модель входит во все включённые форматы; необязательный массив `api` у модели может ограничить их: `[]` исключает модель из GraphQL и OpenAPI, но REST продолжает работать. Если ни одна модель не включает запрошенный формат, генерация завершится понятной ошибкой. Связанные модели должны разрешать тот же формат. Имена моделей должны быть допустимыми идентификаторами; конфликты типов и операций вызывают ошибку. Имена `and`, `or`, `not` зарезервированы фильтрами.

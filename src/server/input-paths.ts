 ...
 */
export function inputPaths(config: { database?: unknown; files?: unknown; auth?: unknown; package?: unknown }, directory = '.', sourcePath?: string): string[] {
  ...
  if (typeof database.source === 'string') {
    ...
    paths.push(databaseLockPath(database.source), `${databaseLockPath(database.source)}.recovery`);
  }
